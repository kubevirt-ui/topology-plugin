...
import { ALL_APPLICATIONS_KEY, TYPE_SERVICE_BINDING, TYPE_WORKLOAD } from '../../const';
...
import { DEFAULT_TOPOLOGY_FILTERS, EXPAND_GROUPS_FILTER_ID } from '../../filters';
import { MockResources } from '../../utils/__tests__/test-resource-data';
import { getImageForCSVIcon } from '../../utils/icon-utils';
import { TopologyDisplayFilterType, WorkloadData } from '../../utils/types/topology-types';
...
describe('operator data transformer ', () => {
  ...
  beforeEach(() => {
    mockResources = cloneDeep(MockResources);
    filters = cloneDeep(DEFAULT_TOPOLOGY_FILTERS);
  ...
  it('should return graph nodes for operator backed services', async () => {
    ...
    const operatorBackedServices = graphData.nodes?.filter(
      (node) => node?.type === TYPE_OPERATOR_BACKED_SERVICE,
    );
  ...
  it('should return csv icon for operator backed service', async () => {
    const icon = get(mockResources.clusterServiceVersions.data[0], 'spec.icon.0');
