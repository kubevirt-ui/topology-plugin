...
export const cleanUpWorkload = async (resource: K8sResourceKind): Promise<K8sResourceKind[]> => {
  ...
  const resourceModel = getK8sModel(resource);
  const resources = {
    buildConfigs: {
      data: buildConfigs,
      loaded: true,
  ...
  if (isDynamicEventResourceKind(getGroupVersionKindForResource(resource)))
    deleteRequest(getK8sModel(resource), resource);
