...
import { has, pullAt, remove, size } from '@topology-utils/common-utils';
...
export const updateItemAppConnectTo = (
  ...
  const model = getK8sModel(item);

  if (!model) {
    return Promise.reject(
      new Error(
        i18next.t('plugin__topology-plugin~Unable to retrieve model for: {{kind}}', {
          kind: item.kind,
        }),
      ),
    );
...
const getSourceAndTargetForBinding = async (
  ...
  if (!contextualSource) {
    return Promise.reject(
      new Error(
        i18next.t('plugin__topology-plugin~Cannot do a contextual binding without a source'),
      ),
    );
  }
  ...
  if (serviceBindingAvailable || !Array.isArray(resources)) {
    ...
    target = (resources as K8sResourceKind[]).find((resource) =>
      linkingModelRefs.includes(getReferenceForResource(resource)),
    );
  }
  const {
    metadata: { namespace },
  ...
  if (!source) {
    return Promise.reject(
      new Error(
        i18next.t(
          'plugin__topology-plugin~Cannot find resource ({{contextualSource}}) to do a contextual binding to',
          {
            contextualSource,
          },
        ),
