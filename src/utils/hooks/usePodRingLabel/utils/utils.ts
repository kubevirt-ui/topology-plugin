...
const getTitleAndSubtitle = (
  ...
  if (!currentPodCount) {
    titlePhrase = isPending ? '0' : tFunc('plugin__topology-plugin~Scaled to 0');
    longTitle = !isPending;
    if (desiredPodCount) {
      subTitlePhrase = tFunc('plugin__topology-plugin~Scaling to {{podSubTitle}}', {
  ...
  switch (ownerKind) {
    ...
    case RevisionModel.kind:
      ...
      if (!isPending && !desiredPodCount) {
        podRingLabelData.title = tFunc('plugin__topology-plugin~Autoscaled');
        podRingLabelData.subTitle = tFunc('plugin__topology-plugin~to 0');
        podRingLabelData.reversed = true;
        break;
      }
      if (isPending) {
        podRingLabelData.title = '0';
        podRingLabelData.subTitle = tFunc('plugin__topology-plugin~Scaling to {{podSubTitle}}', {
          podSubTitle: desiredPodCount,
        });
      } else {
...
export const hpaPodRingLabel = (
  ...
  return {
    title: scaling
      ? tFunc('plugin__topology-plugin~Autoscaling')
      : tFunc('plugin__topology-plugin~Autoscaled'),
    subTitle: tFunc('plugin__topology-plugin~to {{count}} Pod', { count: desiredPods }),
    longTitle: false,
    longSubtitle: false,
    reversed: true,
  };
