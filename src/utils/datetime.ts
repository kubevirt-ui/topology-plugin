...
import { getLastLanguage } from '@topology-utils/common-utils';
...
export const fromNow = (dateTime: string | Date, now?: Date, options?) => {
  ...
  const justNow = i18n.t('plugin__topology-plugin~Just now');
  ...
  if (options?.omitSuffix) {
    if (days) {
      return i18n.t('plugin__topology-plugin~{{count}} day', { count: days });
    }
    if (hours) {
      return i18n.t('plugin__topology-plugin~{{count}} hour', { count: hours });
    }
    return i18n.t('plugin__topology-plugin~{{count}} minute', { count: minutes });
  }
