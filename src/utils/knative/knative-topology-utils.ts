...
import { getName, isEmpty } from '@topology-utils/common-utils';
...
export const getSubscribedPubSubNodes = (
  ...
  pubsubConnectors.forEach((connector: string) => {
    if (resources[connector] && resources[connector].data.length > 0) {
      const pubsubConnectorResources = resources[connector].data as K8sResourceKind[];
...
export const createPubSubDataItems = (
  ...
  const triggerList = (resources?.triggers?.data ?? []) as K8sResourceKind[];
  let triggersData = {};
...
export const getSinkUriTopologyEdgeItems = (
  ...
  if (sinkUri && uid) {
    edges.push({
      ...
      label: i18next.t('plugin__topology-plugin~Event source connector'),
      source: uid,
      target: targetUid,
    });
  }
    ...
    return [
      {
        ...
        label: i18next.t('plugin__topology-plugin~Event sink connector'),
        target: targetUid,
  ...
  const edges = data.reduce((acc, kafkaConnection) => {
    ...
    if (
      ...
      acc.push({
        ...
        label: i18next.t('plugin__topology-plugin~Kafka connector'),
    ...
    if (resUid) {
      ...
      if (revisionIndex >= 0) {
        ...
        edges.push({
          ...
          label: i18next.t('plugin__topology-plugin~Traffic distribution connector'),
          source: uid,
          target: resUid,
...
export const createTopologyServiceNodeData = (
  ...
  return {
    ...
    resources: { ...svcRes },
    data: {
      url: (knativeSvc as K8sResourceKind).status?.url || '',
...
export const createTopologyPubSubNodeData = (
  ...
  return {
    ...
    resources: { ...res },
    data: {
      kind: getReferenceForResource(res.obj),
      isKnativeResource: true,
    },
  };
};
