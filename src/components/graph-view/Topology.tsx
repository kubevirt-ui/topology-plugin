...
import { isTopologyComponentFactory, TopologyComponentFactory } from '../../extensions';
import { getTopologyGraphModel, setTopologyGraphModel } from '../../redux/action';
import useQueryParams from '../../utils/hooks/useQueryParams';
import { useUserSettingsCompatibility } from '../../utils/hooks/useUserSettingsCompatibility/useUserSettingsCompatibility';
import {
...
import { DEFAULT_LAYOUT, layoutFactory, SUPPORTED_LAYOUTS } from './layouts/layoutFactory';
...
const Topology: FC<TopologyProps & StateProps & DispatchProps> = ({
  model,
  application,
  namespace,
  onSelect,
  ...
  const [topologyLayoutData, setTopologyLayoutData] = useUserSettingsCompatibility(
    TOPOLOGY_LAYOUT_CONFIG_STORAGE_KEY,
    TOPOLOGY_LAYOUT_LOCAL_STORAGE_KEY,
    {},
  );
  const topologyLayoutDataJson = useMemo(() => topologyLayoutData || {}, [topologyLayoutData]);
  ...
  const [componentFactoryExtensions, isStaticResolved] =
    useResolvedExtensions<TopologyComponentFactory>(isTopologyComponentFactory);
  const [dynamicComponentFactoryExtensions, isDynamicResolved] =
    useResolvedExtensions<DynamicTopologyComponentFactory>(isDynamicTopologyComponentFactory);
  ...
  useEffect(() => {
    if (model && visualizationReady) {
      ...
      model.nodes.forEach((n) => {
        ...
        if (oldNode && isEqual(oldNode.getData(), n.data)) {
          n.data = oldNode.getData();
        }
      });
      model.edges.forEach((e) => {
        const oldEdge = visualization.getEdgeById(e.id);
        if (oldEdge && isEqual(oldEdge.getData(), e.data)) {
