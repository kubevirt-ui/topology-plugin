    ...
    if (
      ...
      const title =
        failureTitle !== undefined
          ? failureTitle
          : i18next.t('plugin__topology-plugin~Error moving connection');
...
const withNoDrop = () => {
  return withDndDrop<any, any, {}, NodeComponentProps>(noDropTargetSpec); // eslint-disable-line
};
...
const createVisualConnector = (source: Node, target: Node | Graph): ReactElement[] | null => {
  ...
  createConnection(source, target, null).catch((error) => {
    errorModal({
      title: i18next.t('plugin__topology-plugin~Error creating connection'),
      error: error.message,
    });
  });
