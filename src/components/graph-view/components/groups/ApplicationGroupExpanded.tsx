...
import { maxBy, minBy } from '@topology-utils/common-utils';
...
export function computeLabelLocation(points: PointWithSize[]): PointWithSize {
  ...
  return [
    (minBy(lowPoints, (p) => p[0])[0] + maxBy(lowPoints, (p) => p[0])[0]) / 2,
    lowPoints[0][1],
    // use the max size value
    maxBy(lowPoints, (p) => p[2])[2],
  ];
}

const ApplicationGroupExpanded: FunctionComponent<ApplicationGroupExpandedProps> = ({
  className,
  element,
  collapsible,
  selected,
  ...
  const labelLocation = useRef<PointWithSize>();
