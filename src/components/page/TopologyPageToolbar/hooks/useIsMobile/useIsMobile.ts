...
const useIsMobile = () => {
  const [isMobile, setIsMobile] = useState<boolean>((window as any).innerWidth <= MAX_MOBILE_WIDTH);

  useEffect(() => {
    const mobileResolutionMatch = (window as any).matchMedia(MOBILE_MEDIA_QUERY);
