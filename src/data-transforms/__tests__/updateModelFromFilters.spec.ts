...
import { MockResources } from '@topology-utils/__tests__/test-resource-data';
import { getReferenceForResource } from '@topology-utils/k8s-utils';

import { dataModel, TEST_KINDS_MAP, topologyDataModel } from '../../__tests__/topology-test-data';
import { ALL_APPLICATIONS_KEY, TYPE_WORKLOAD } from '../../const';
import { EXPAND_GROUPS_FILTER_ID, getFilterById } from '../../filters';
import { DEFAULT_TOPOLOGY_FILTERS, EXPAND_APPLICATION_GROUPS_FILTER_ID } from '../../filters';
...
describe('topology model ', () => {
  ...
  it('should remove filtered kinds', () => {
    ...
    filters.push({
      ...
      id: getReferenceForResource(MockResources.deployments.data[0]),
    ...
    filters.push({
      type: TopologyDisplayFilterType.kind,
      id: getReferenceForResource(MockResources.cronJobs.data[0]),
      label: 'DeploymentConfig',
      priority: 1,
      value: true,
    });
